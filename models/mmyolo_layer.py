...
class MMYOLOLayer(nn.Module):
    ...
    def __init__(
        ...
        super(MMYOLOLayer, self).__init__()
        ...
        self.conv0 = nn.Conv2d(in_channels=in_ch,
                              out_channels=channels_per_anchor * self.n_anchors,
                              kernel_size=1, stride=1, padding=0)
        self.conv1 = nn.Conv2d(in_channels=in_ch,
                              out_channels=channels_per_anchor * self.n_anchors,
                              kernel_size=1, stride=1, padding=0)
        self.conv = nn.Conv2d(in_channels=channels_per_anchor * self.n_anchors * 2,
                              out_channels=channels_per_anchor * self.n_anchors,
                              kernel_size=1, stride=1, padding=0)

    def forward(self, xin0, xin1, labels=None):
        """
        In this
        Args:
        ...
        """
        out0 = self.conv0(xin0)
        out1 = self.conv1(xin1)
        ...
        x_shift = torch.tensor(np.broadcast_to(
            np.arange(fsize, dtype=np.float32), output.shape[:4]), device=output.device)
        y_shift = torch.tensor(np.broadcast_to(
            np.arange(fsize, dtype=np.float32).reshape(fsize, 1), output.shape[:4]), device=output.device)

        masked_anchors = np.array(self.masked_anchors)

        w_anchors = torch.tensor(np.broadcast_to(np.reshape(
            masked_anchors[:, 0], (1, self.n_anchors, 1, 1)), output.shape[:4]), device=output.device)
        h_anchors = torch.tensor(np.broadcast_to(np.reshape(
            masked_anchors[:, 1], (1, self.n_anchors, 1, 1)), output.shape[:4]), device=output.device)
        ...
        tgt_mask = torch.zeros(batchsize, self.n_anchors,
                               fsize, fsize, 4 + self.n_classes).to(output.device)
        obj_mask = torch.ones(batchsize, self.n_anchors,
                              fsize, fsize).to(output.device)
        tgt_scale = torch.zeros(batchsize, self.n_anchors,
                                fsize, fsize, 2).to(output.device)

        target = torch.zeros(batchsize, self.n_anchors,
                             fsize, fsize, n_ch).to(output.device)
        ...
        for b in range(batchsize):
            ...
            if n == 0:
                continue
            truth_box = torch.tensor(np.zeros((n, 4))).to(output.device)
